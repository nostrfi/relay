...
func seedEvent(t *testing.T, ctx context.Context, repo *Repository, expiration *int64) {
	t.Helper()
...
func seedEventAt(t *testing.T, ctx context.Context, repo *Repository, createdAt nostr.Timestamp, expiration *int64) {
	t.Helper()
	...
	require.NoError(t, err)
	ok, err := repo.SaveEvent(ctx, wrapped)
	require.NoError(t, err)
	require.True(t, ok)
}
...
func TestBackupRestoreRoundTrip(t *testing.T) {
	...
	sourceRepo := &Repository{db: sourceDB}
	...
	restoredRepo := &Repository{db: targetDB}
