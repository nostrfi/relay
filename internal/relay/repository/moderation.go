...
func (r *duckDBRepository) BanPubkey(ctx context.Context, pubkey, reason string) error {
...
func (r *duckDBRepository) UnbanPubkey(ctx context.Context, pubkey string) error {
	_, err := r.db.ExecContext(ctx, "DELETE FROM banned_pubkeys WHERE pubkey = ?", pubkey)
	return err
}

func (r *duckDBRepository) ListBannedPubkeys(ctx context.Context) ([]ModerationEntry, error) {
	return listModerationEntries(ctx, r.db, "SELECT pubkey, reason, banned_at FROM banned_pubkeys ORDER BY banned_at")
}

func (r *duckDBRepository) IsPubkeyBanned(ctx context.Context, pubkey string) (bool, error) {
	var exists bool
	err := r.db.QueryRowContext(ctx, "SELECT EXISTS(SELECT 1 FROM banned_pubkeys WHERE pubkey = ?)", pubkey).Scan(&exists)
	return exists, err
}

func (r *duckDBRepository) BanEvent(ctx context.Context, eventID, reason string) error {
	_, err := r.db.ExecContext(ctx, `
...
func (r *duckDBRepository) AllowEvent(ctx context.Context, eventID string) error {
	_, err := r.db.ExecContext(ctx, "DELETE FROM banned_events WHERE event_id = ?", eventID)
	return err
}

func (r *duckDBRepository) ListBannedEvents(ctx context.Context) ([]ModerationEntry, error) {
	return listModerationEntries(ctx, r.db, "SELECT event_id, reason, banned_at FROM banned_events ORDER BY banned_at")
}

func (r *duckDBRepository) BlockIP(ctx context.Context, ipOrCIDR, reason string) error {
	_, err := r.db.ExecContext(ctx, `
...
func (r *duckDBRepository) UnblockIP(ctx context.Context, ipOrCIDR string) error {
	_, err := r.db.ExecContext(ctx, "DELETE FROM blocked_ips WHERE ip_or_cidr = ?", ipOrCIDR)
	return err
}

func (r *duckDBRepository) ListBlockedIPs(ctx context.Context) ([]ModerationEntry, error) {
	return listModerationEntries(ctx, r.db, "SELECT ip_or_cidr, reason, blocked_at FROM blocked_ips ORDER BY blocked_at")
}

func listModerationEntries(ctx context.Context, db *sql.DB, query string) ([]ModerationEntry, error) {
