...
- **`websocket.mode`**: `development` (default) accepts every `Origin` — the right mode for a relay meant to serve arbitrary web clients, and re-decided as the default under nostrfi/workspace#45 with the dashboard considered. The relay logs a startup warning whenever this mode binds a non-loopback interface directly — note this detects direct public binds only; a loopback bind behind a same-host reverse proxy or tunnel is still internet-facing and does not warn. `production` restricts **browser** connections: the relay refuses to start unless `websocket.allowed_origins` is non-empty, and a WebSocket upgrade whose `Origin` header is present but malformed or unlisted is rejected with `403 Forbidden`. An upgrade with **no** `Origin` header is admitted in both modes — only browsers send one, so refusing its absence excluded every CLI client, `wscat`, and relay-to-relay sync while stopping no attacker (a non-browser client can forge any `Origin`); this reverses the "missing" half of the original rule from workspace #4. Origin restriction binds honest browsers only — it is a cross-site-misuse control, not an access control.
